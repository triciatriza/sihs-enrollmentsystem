  ...
  async function getStudents() {
    try {
      const studentData = await axios({
        ...
        .then((student) => {
          setStudents((students = student.data));
        })
        .catch((error) => {
          Swal.fire({
            icon: "error",
      ...
      <div className="information">
        ...
        >
          ...
          {students.map((currentStudent) => {
            return (
              <option key={currentStudent.id} value={currentStudent.id}>
                {String(currentStudent.user.firstName) +
                  " " +
                  String(currentStudent.user.lastName)}
              </option>
            );
          })}
        </select>
